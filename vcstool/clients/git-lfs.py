...
class GitLfsClient(VcsClientBase):
    ...
    def __init__(self, path):
        super(GitLfsClient, self).__init__(path)

    def branch(self, command):
        self._check_executable()
        cmd = [GitLfsClient._executable, 'branch']
    ...
    def custom(self, command):
        self._check_executable()
        cmd = [GitLfsClient._executable] + command.args
        return self._run_command(cmd)

    def diff(self, command):
        self._check_executable()
        cmd = [GitLfsClient._executable, 'diff']
    ...
    def export(self, command):
        ...
        if not exact:
            ...
            cmd_branch = [
                GitLfsClient._executable, 'rev-parse', '--abbrev-ref', 'HEAD']
        ...
        if not exact:
            ...
            cmd_remote = [
                GitLfsClient._executable, 'rev-parse', '--abbrev-ref',
                '@{upstream}']
            ...
            # determine the hash
            cmd_ref = [GitLfsClient._executable, 'rev-parse', 'HEAD']
            ...
            cmd_remotes = [GitLfsClient._executable, 'remote']
            ...
            for remote in remotes:
                # get all remote names
                cmd_refs = [
                    GitLfsClient._executable, 'rev-list', '--remotes=' + remote,
                    '--tags']
    ...
    def _get_remote_url(self, remote):
        cmd_url = [
            GitLfsClient._executable, 'config', '--get', 'remote.%s.url' % remote]
    ...
    def import_(self, command):
        ...
        self._check_executable()
        if GitLfsClient.is_repository(self.path):
        ...
        if GitLfsClient.is_repository(self.path):
            if command.version:
                ...
                cmd_remote = [GitLfsClient._executable, 'rGitLfsClient._executable
                # override locale in order to parse outputGitLfsClient._executable
                env = os.environ.copy()
                env['LC_ALL'] = 'C'
            ...
            cmd_fetch = [GitLfsClient._executable, 'fetch', remote]
            ...
            if command.recursive:
                cmd_submodule = [
                    GitLfsClient._executable, 'submodule', 'update', '--init']
            ...
            cmd_clone = [GitLfsClient._executable, 'clone']
            if command.recursive:
        ...
        if checkout_version:
            cmd_checkout = [
                GitLfsClient._executable, 'checkout', checkout_version]
    ...
    def _get_remote_urls(self):
        cmd_remote = [GitLfsClient._executable, 'remote', 'show']
        result_remote = self._run_command(cmd_remote)
        if result_remote['returncode']:
    ...
    def log(self, command):
        ...
        if command.limit_tag:
            ...
            cmd_tag = [GitLfsClient._executable, 'tag', '-l', command.limit_tag]
            ...
            cmd = [GitLfsClient._executable, 'log', '%s..' % command.limit_tag]
        elif command.limit_untagged:
            # determine nearest tag
            cmd_tag = [
                GitLfsClient._executable, 'describe', '--abbrev=0', '--tags']
            result_tag = self._run_command(cmd_tag)
            if result_tag['returncode']:
                return result_tag
            # output log since nearest tag
            cmd = [GitLfsClient._executable, 'log', '%s..' % result_tag['output']]
        else:
            cmd = [GitLfsClient._executable, 'log']
        if command.limit != 0:
            cmd += ['-%d' % command.limit]
        self._check_color(cmd)
        return self._run_command(cmd)
    ...
    def pull(self, _command):
        self._check_executable()
        cmd = [GitLfsClient._executable, 'pull']
        self._check_color(cmd)
        return self._run_command(cmd)

    def push(self, _command):
        self._check_executable()
        cmd = [GitLfsClient._executable, 'push']
        return self._run_command(cmd)

    def remotes(self, _command):
        self._check_executable()
        cmd = [GitLfsClient._executable, 'remote', '-v']
        return self._run_command(cmd)
    ...
    def status(self, command):
        ...
        while command.hide_empty:
            # check if ahead
            cmd = [GitLfsClient._executable, 'log', '@{push}..']
            result = self._run_command(cmd)
            if not result['returncode'] and result['output']:
                # ahead, do not hide
                break
            # check if behind
            cmd = [GitLfsClient._executable, 'log', '..@{upstream}']
            result = self._run_command(cmd)
            if not result['returncode'] and result['output']:
                # behind, do not hide
                break
            cmd = [GitLfsClient._executable, 'status', '-s']
            if command.quiet:
            ...
            if result['returncode'] or not result['output']:
                return result
            break
        cmd = [GitLfsClient._executable, 'status']
        self._check_color(cmd)
        if command.quiet:
            cmd += ['--untracked-files=no']
        return self._run_command(cmd)
    ...
    def _check_color(self, cmd):
        ...
        if GitLfsClient._config_color_is_auto is None:
            _cmd = [GitLfsClient._executable, 'config', '--get', 'color.ui']
            result = self._run_command(_cmd)
            GitLfsClient._config_color_is_auto = result['output'] in ['', 'auto']

        # inject arguments to force colorization
        if GitLfsClient._config_color_is_auto:
            cmd[1:1] = '-c', 'color.ui=always'

    def _check_executable(self):
        assert GitLfsClient._executable is not None, \
            "Could not find 'git' executable"


if not GitLfsClient._executable:
    GitLfsClient._executable = which('git-lfs')
